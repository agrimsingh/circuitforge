...
| Performance | B+ | Self-hosted local compiler (no external timeout), parallelized compile/validation pipeline, speculative compilation, parallel KiCad analyses, phase-scoped tool access, lazy guardrail fetch, SSE heartbeat keepalive, abort signal propagation |
